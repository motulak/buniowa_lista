...
class TestClassUser(unittest.TestCase):
    ...
    def test_Create_New_User_old_list(self):
        user1 = User('Adam', '123')
        self.assertEqual(user1.name, 'Adam')
        self.assertEqual(user1.paswd, '123')
        self.assertEqual(user1.id, 0)
        self.assertEqual(user1.lista.elements, [])
        print(user1.lista.update_date)
